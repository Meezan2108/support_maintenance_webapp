<?php

namespace Database\Seeders;

use App\Helpers\EtlHelper;
use App\Models\RefSeoCategory;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class RefSeoCategorySeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        RefSeoCategory::truncate();
        info("START SEED " . __CLASS__);

        DB::unprepared('SET IDENTITY_INSERT ' . (new RefSeoCategory())->getTable() . ' ON');

        $file = fopen(storage_path("csv/crims-db-v1/refSEOCategory.csv"), 'r');

        $hasHeader = true;

        $isHeaderSkiped = false;
        $arrInsert = [];
        while (($row = fgetcsv($file)) !== FALSE) {
            if ($hasHeader && !$isHeaderSkiped) {
                $isHeaderSkiped = true;
                continue;
            }
            $arrInsert[] = $this->transform($row);
        }

        RefSeoCategory::insert($arrInsert);

        DB::unprepared('SET IDENTITY_INSERT ' . (new RefSeoCategory())->getTable() . ' OFF');

        fclose($file);

        info("FINISH SEED " . __CLASS__);
    }


    protected function transform($oldRow)
    {
        $oldRow = collect($oldRow);
        $oldRow = $oldRow->map(function ($item) {
            $item = trim($item);
            return EtlHelper::transformNull($item);
        });

        return [
            "id" => $oldRow[0],
            "code" => $oldRow[2],
            "description" => $oldRow[1],
            "ref_seo_sector_id" => $oldRow[3],
            "detail" => $oldRow[4],
            "created_at" => now(),
            "updated_at" => now()
        ];
    }
}
