...
class GetCommercializationDatatables
{
    protected $columns = [];
    ...
    public function __construct()
    {
        $this->columns = [
            ...
            [
                "label" => "Updated At",
                "name" => "commercialization.updated_at",
                "orderable" => true,
                "isDateTime" => true,
                "class" => "text-nowrap"
            ]
    ...
    public function execute(array $filters)
    {
        ...
        $refOutputType = new RefOutputType();

        $data = Commercialization::with("kpiAchievement.user")
            ->join(
                $kpiAchievement->getTable(),
                function ($join) use ($commercialization, $kpiAchievement) {
                    $join->on($commercialization->qualifyColumn("id"), $kpiAchievement->qualifyColumn("reff_id"));
                    $join->where($kpiAchievement->qualifyColumn("reff_type"), Commercialization::class);
                }
            )
            ->join(
                $user->getTable(),
            ...
            ->join(
                $refOutputType->getTable(),
                function ($join) use ($commercialization, $refOutputType) {
                    $join->on($commercialization->qualifyColumn("category"), $refOutputType->qualifyColumn("id"));
                }
            )
            ...
            ->orderBy(
                $filters["order_by"] ?? $commercialization->qualifyColumn('updated_at'),
                $filters["order_type"] ?? 'desc'
            )
            ->select(
                $commercialization->qualifyColumn("id"),
                $commercialization->qualifyColumn("date"),
                $commercialization->qualifyColumn("name as name_product"),
                $commercialization->qualifyColumn("user_id"),
                $user->qualifyColumn('name'),
                $kpiAchievement->qualifyColumn("approval_status"),
                $commercialization->qualifyColumn("updated_at"),
            )
