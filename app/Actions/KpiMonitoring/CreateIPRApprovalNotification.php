...
class CreateIPRApprovalNotification
{
    /**
     * Execute the action
     *
     ...
     * @return any
     */
    public function execute(IPR $kpi, User $user, int $approvementStatus)
    {
        $showLink = route("panel.ipr.show", ["ipr" => $kpi->id]);
        $editLink = route("panel.ipr.edit", ["ipr" => $kpi->id]);

        $title = "IPR : " . $kpi->output;
