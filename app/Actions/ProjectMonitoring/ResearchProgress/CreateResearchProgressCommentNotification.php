...
class CreateResearchProgressCommentNotification
{
    ...
    public function execute(ReportResearchProgress $report, User $user, int $approvementStatus)
    {
        ...
        $title = "Research Progress Report : " . $report->proposal->project_title;
        ...
        if ($targetRoleId != User::ROLE_DIVISION_DIRECTOR) {
            ...
            SendEmailNotificationReport::dispatch(
                "Research Progress Report has reviewed",
                $notification->id
            );
        }
        ...
        if (in_array($report->approval_status, [Approvement::STATUS_APPROVED, Approvement::STATUS_REJECTED])) {
            ...
            (new CreateNotification)->execute($report, Notifable::TARGET_TYPE_USER, $report->proposal->user_id, Template::TYPE_REPORT_STATUS_UPDATED, $options);
