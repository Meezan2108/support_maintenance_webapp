...
class ShowResource extends JsonResource
{
    ...
    public function getIdentification()
    {
        return [
            ...
            'type_of_funding' => $this->typeOfFund,
            'project_title' => $this->project_title,
            'user_id' => $this->user_id,
            "user" => $this->user,
            'researcher' => optional($this->researcher)->load("position", "division"),
    ...
    public function getObjectives() // form2
    {
        return [
            ...
            'research_cluster' => $this->researchCluster,

            'objectives' => $this->objectives,

            'ref_seo_category_id' => $this->ref_seo_category_id,
            'seo_category' => $this->seoCategory,

            'ref_seo_group_id' => $this->ref_seo_group_id,
            'seo_group' => $this->seoGroup,

            'ref_seo_area_id' => $this->ref_seo_area_id,
            'seo_area' => $this->seoArea,

            'for_primary' => optional($this->primaryResearchField)->load('category', 'group', 'area'),
            'for_secondary' => optional($this->secondaryResearchField)->load('category', 'group', 'area')
        ];
    }
    ...
    public function getResearchCollaboration()
    {
        ...
        return [
            "organizations" => $collaborations->where("type", 1),
            "industries" => $collaborations->where("type", 2),
            "project_leaders" => $teams->where("type", 1),
            "researchers" => $teams->where("type", 2),
            "staffs" => $teams->where("type", 3)
        ];
    }
