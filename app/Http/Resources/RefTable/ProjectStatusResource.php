...
class ProjectStatusResource extends JsonResource
{
    ...
    private function getArrButton()
    {
        $show = [
            "icon" => "fas fa-info",
            "url" => route("panel.ref-table.project-status.show", ["status" => $this->id]),
        ...
        $edit = [
            "icon" => "fas fa-edit",
            "url" => route("panel.ref-table.project-status.edit", ["status" => $this->id]),
            "label" => "Edit Project Status",
            "classStyle" => "btn-outline-warning"
        ];

        $delete = [
            "icon" => "fas fa-trash",
            "url" => route("panel.ref-table.project-status.delete", ["status" => $this->id]),
