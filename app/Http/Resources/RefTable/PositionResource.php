...
class PositionResource extends JsonResource
{
    ...
    private function getArrButton()
    {
        $show = [
            "icon" => "fas fa-info",
            "url" => route("panel.ref-table.position.show", ["position" => $this->id]),
            "label" => "Show Detail Research Type",
            "classStyle" => "btn-outline-info"
        ];

        $edit = [
            "icon" => "fas fa-edit",
            "url" => route("panel.ref-table.position.edit", ["position" => $this->id]),
            "label" => "Edit Research Type",
            "classStyle" => "btn-outline-warning"
        ];

        $delete = [
            "icon" => "fas fa-trash",
            "url" => route("panel.ref-table.position.delete", ["position" => $this->id]),
            "label" => "Delete Research Type",
