...
class OtherDocumentResource extends JsonResource
{
    ...
    private function getArrButton()
    {
        $show = [
            "icon" => "fas fa-info",
            "url" => route("panel.ref-table.other-document.show", ["document" => $this->id]),
            "label" => "Show Detail Other Document",
            "classStyle" => "btn-outline-info"
        ];

        $edit = [
            "icon" => "fas fa-edit",
            "url" => route("panel.ref-table.other-document.edit", ["document" => $this->id]),
            "label" => "Edit Other Document",
            "classStyle" => "btn-outline-warning"
        ];

        $delete = [
            "icon" => "fas fa-trash",
            "url" => route("panel.ref-table.other-document.delete", ["document" => $this->id]),
            "label" => "Delete Other Document",
