...
use App\Actions\ProjectMonitoring\CheckExistingQuarterlyReport;
...
use App\Actions\ProjectMonitoring\Qfr\UpdateFinancialProgress;
use App\Http\Controllers\Controller;
use App\Http\Requests\ProjectMonitoring\Mar\Form1Request;
use App\Http\Requests\ProjectMonitoring\Qfr\Form2Request;
...
class Form2FinancialProgressController extends Controller
{
    ...
    public function store(
        Form2Request $request,
        CheckExistingQuarterlyReport $checkExistingReport,
        UpdateFinancialProgress $updateFinancialProgress
    ) {
        ...
        if ($reportExist = $checkExistingReport->execute($proposal, $arrData["year"], $arrData["quarter"], ReportQuarterly::TYPE_QFR)) {
            $url = $proposal->proposal_type == Proposal::TYPE_TRF
                ? route("panel.monitoring-trf.qfr.show", ["qfr" => $reportExist->id])
                : "";
        ...
        $report = DB::transaction(function () use (
            ...
            $updateFinancialProgress->execute($report, $arrData);
            return $report;
        });
    ...
    public function update(
        ...
        $report = DB::transaction(function () use ($report, $arrData, $updateFinancialProgress) {
