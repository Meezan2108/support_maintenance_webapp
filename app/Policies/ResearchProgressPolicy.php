...
class ResearchProgressPolicy
{
    ...
    {
        return $user->can("research-progress-list");
    }
    ...
    public function view(User $user, ReportResearchProgress $model): bool
    {
        if ($user->hasRole(['Super Admin', 'Division Director', 'LKM Director', 'R&D Coordinator', 'RMC'])) {
            return $user->can("research-progress-show");
        }

        return $user->can("research-progress-show") && $user->id == $model->proposal->user_id;
    }
    ...
    public function create(User $user, Proposal $model = null): bool
    {
        if ($user->hasRole(['Super Admin']) || !$model) {
            return $user->can("research-progress-create");
        }
    ...
    public function update(User $user, ReportResearchProgress $model): bool
    {
        if ($user->hasRole(['Super Admin'])) {
            return $user->can("research-progress-edit");
        }
    ...
    {
        if ($user->hasRole(['Super Admin'])) {
            return $user->can("research-progress-approval");
        }
        ...
        if (!$user->can("research-progress-approval")) {
            return false;
        }
    ...
    {
        if ($user->hasRole(['Super Admin'])) {
            return $user->can("research-progress-delete");
        }
